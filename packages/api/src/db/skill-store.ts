...
export function createDbWorkspaceSkillStore(hooks?: WorkspaceSkillStoreHooks): WorkspaceSkillStore {
  ...
  return {
    ...
    async create(userId, workspaceId, input) {
      ...
      // Procedural-brain governance (mig 260, plan §5.2–5.3). Authored skills are
      // born active (confidence 1.0); auto-induced skills are born SUGGESTED —
      // confidence 0.0, activated_at NULL — and earn activation only via human
      // confirmation or independent re-derivation (never silently active).
      const inductionSource = input.inductionSource ?? (isAuto ? 'self' : 'authored')
      const confidence = isAuto ? 0.0 : 1.0
      const activatedAt = isAuto ? null : new Date()
    ...
    async incrementSucceeded(skillId) {
      await query(
        `UPDATE workspace_skills SET succeeded = succeeded + 1 WHERE id = $1`,
        [skillId],
      )
    },
    ...
    async recordRederivation(skillId) {
      // Bump count + confidence; activate when the threshold is reached, EXCEPT
      // ingested-source skills (never auto-active — they require confirmSkill).
      await query(
        `UPDATE workspace_skills
         SET rederivation_count = rederivation_count + 1,
             ...
             updated_at = now()
         WHERE id = $1`,
        [skillId, SKILL_REDERIVATION_INCREMENT, SKILL_ACTIVATION_THRESHOLD],
      )
    },
  }
}
