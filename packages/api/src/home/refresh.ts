...
- needsYou: order the action cards ('brain_review', 'approvals', 'autopilot', 'task_cleanup', 'deal_attention', 'connector_attention', 'workflow_attention') by what matters most right now. A broken connector or failed workflow runs usually belong first — they silently block everything downstream. Omit a kind whose count is already 0. Every kind EXCEPT 'brain_review', 'task_cleanup', and 'deal_attention' (non-blocking hygiene) surfaces automatically while its count is live even if you omit it (attention kinds and pending-you actions like approvals/autopilot are never hidden); list one only to reposition or caption it. Counts are filled in live, so never state a number yourself.
