...
export async function injectSkills(opts: InjectSkillsOptions): Promise<InjectSkillsResult> {
  ...
  // mig 492: slugs whose `workspace_skills.all_assistants` is set — offered to
  ...
  try {
    ...
    if (opts.workspaceSkillStore && opts.workspaceId) {
      try {
        ...
        for (const ws of workspaceSkills) {
          ...
          // mig 492: the OTHER half of offering scope. The allowlist can only
    ...
    // four. `allAssistantsSlugs` (mig 492) is the only one that survives the
