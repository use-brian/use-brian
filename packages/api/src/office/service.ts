...
export function createOfficeService(deps: OfficeServiceDeps): OfficeToolPort {
  return {
    ...
    async get(params) {
      ...
      return { artifactId: artifact.id, family: artifact.family, title: artifact.title, version: artifact.headVersion, lifecycleState: artifact.lifecycleState === 'purged' ? 'retained' : artifact.lifecycleState, role: access.role, job: job ? { id: job.id, status: job.status, stage: job.stage } : undefined }
    },
