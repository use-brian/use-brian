 ...
 *
 * Usage:
 *   pnpm --filter @sidanclaw/api token-report
 *
...
async function main() {
  ...
  const body = [
    ...
    '',
    '> This file is regenerated by `pnpm --filter @sidanclaw/api token-report`. Commit updates alongside the change that shifted the numbers. Live regression guards live in `packages/api/src/routes/__tests__/prompt-token-cost.test.ts`; scenario definitions in `packages/api/src/routes/_token-cost-scenarios.ts`.',
    '',
    `- **Generated:** ${today} (UTC)`,
    `- **Branch:** \`${branch}\``,
    `- **Commit:** \`${commit}\``,
