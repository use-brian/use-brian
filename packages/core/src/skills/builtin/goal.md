...
3. **Budget from the user's words.** Map "$20" → `max_spend`, "by Friday" → `deadline`, "max 10 tries" → `max_iterations`. If none given, omit them - the defaults (30 iterations / $5) apply at kickoff. Mention the budget in your reply so the user can object.
