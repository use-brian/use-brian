...
**Chat + channels.** Web chat, Telegram, Slack, WhatsApp, Feishu/Lark. Group chats are mention-gated.
...
- Event sources — exactly: `connector` (a connected connector instance, e.g. GitHub/Gmail/Calendar/Fathom ingest), `channel` (a connected Slack/Telegram/WhatsApp/Feishu integration), `page` (a doc page + its direct children), `task` (the workspace's tasks; lifecycle actions matched via `match.inChannels`: created / completed / blocked / reopened / assigned / tagged / updated; assistant-created tasks need `fromBots: true`), `knowledge` (the workspace knowledge base; actions created / updated / deleted, entry tags via `match.tags`), `brand` (the workspace brand record; actions created / updated / approved / superseded, the brand slug via `match.tags`; assistant-proposed drafts need `fromBots: true`), `crm` (committed CRM operations only; exact event types are `crm.submission.received`, `crm.submission.updated`, `crm.consent.changed`, `crm.suppression.changed`, `crm.entitlement.changed`, `crm.participation.changed`, and `crm.deal.stage_changed`; use `match.inChannels` for event types and `match.tags` for enumerated stable definition/purpose/plan/event/pipeline/stage keys).
- Delivery — `telegram` / `slack` / `whatsapp` / `feishu` only. **Never web** (the web app is pull-only) — for a web-visible result, write to a doc page via the step's `page` anchor.
...
- Trigger kinds or event sources beyond the exact sets above. CRM emits only the seven committed operations listed above, not arbitrary contact/company/deal field changes. The web chat itself is not a channel event source.
