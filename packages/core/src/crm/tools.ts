...
export function createCrmTools(
  ...
  const saveContact = buildTool({
    ...
    description:
      'Upsert a contact in the current workspace. Dedupes on email first (case-insensitive), then a structured external_ref provider + id, then exact display name or a learned alias. An existing active match receives merged tags (union), non-empty phone / email / company_id (incoming wins), and external_ref (shallow merge). Dedupe and reads are scoped to entities visible to you: a matching contact another member keeps private is never merged into — you get your own row. Use updateContact when you have an explicit id and need to patch other fields. ' +
