...
describe('[COMP:office/pptx-engine] PPTX engine', () => {
  ...
  it('normalizes a conventional external PPTX and rejects external media relationships', async () => {
    ...
    zip.file('ppt/slides/_rels/slide1.xml.rels', '<Relationships><Relationship Id="rLayout" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/></Relationships>')
    zip.file('ppt/slideLayouts/slideLayout1.xml', '<p:sldLayout xmlns:p="p"><p:cSld name="Title layout"><p:spTree/></p:cSld></p:sldLayout>')
    zip.file('ppt/slideLayouts/_rels/slideLayout1.xml.rels', '<Relationships><Relationship Id="rMaster" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideMaster" Target="../slideMasters/slideMaster1.xml"/></Relationships>')
    zip.file('ppt/slideMasters/slideMaster1.xml', '<p:sldMaster xmlns:p="p" xmlns:a="a"><p:cSld name="Brand master"><p:spTree><p:sp><p:spPr><a:xfrm><a:off x="457200" y="4572000"/><a:ext cx="2286000" cy="228600"/></a:xfrm><a:prstGeom prst="rect"/><a:noFill/><a:ln><a:noFill/></a:ln></p:spPr><p:txBody><a:p><a:r><a:t>MASTER FOOTER</a:t></a:r></a:p></p:txBody></p:sp></p:spTree></p:cSld></p:sldMaster>')
    ...
    expect(result.snapshot.masters).toContainEqual(expect.objectContaining({ name: 'Brand master' }))
    expect(result.snapshot.layouts).toContainEqual(expect.objectContaining({ name: 'Title layout' }))
    expect(result.snapshot.slides[0].objects).toContainEqual(expect.objectContaining({ kind: 'text', locked: true, runs: expect.arrayContaining([expect.objectContaining({ text: 'MASTER FOOTER' })]) }))
  ...
  it('keeps paragraphs in their source text box and preserves positioned media, tables, and scatter charts', async () => {
    ...
    zip.file('ppt/slides/slide1.xml', `<p:sld xmlns:p="p" xmlns:a="a" xmlns:r="r" xmlns:c="c"><p:cSld><p:spTree>
      ...
      <p:sp><p:nvSpPr><p:cNvPr id="3" name="Label"/></p:nvSpPr><p:spPr><a:xfrm><a:off x="6096000" y="685800"/><a:ext cx="2286000" cy="457200"/></a:xfrm><a:prstGeom prst="rect"/><a:noFill/><a:ln><a:noFill/></a:ln></p:spPr><p:txBody><a:bodyPr anchor="b"/><a:p><a:pPr algn="r"><a:defRPr b="1"/></a:pPr><a:r><a:rPr sz="600" b="0"/><a:t>Separate label</a:t></a:r></a:p></p:txBody></p:sp>
    ...
    expect(text[1]).toEqual(expect.objectContaining({ alignment: 'end', verticalAlignment: 'bottom', runs: [expect.objectContaining({ style: expect.objectContaining({ fontSizePt: 6, bold: false }) })] }))
    ...
    expect(fitOfficeArtifact(result.snapshot, { minimumFontSizePt: 1 }).issues).toEqual([])
  })
})
