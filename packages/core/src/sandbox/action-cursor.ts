/**
 * Best-effort visible cursor for Brian-owned browser actions.
 * [COMP:sandbox/action-cursor]
 *
 * The page's real pointer/focus/input event supplies the position. This code
 * never chooses the action target and never replaces trusted browser input.
 */
...
export function buildActionCursorArmScript(kind: ActionCursorKind): string {
  return `(${ACTION_CURSOR_FUNCTION})(${JSON.stringify(kind)})`
}

export function encodeActionCursorArmScript(kind: ActionCursorKind): string {
  return Buffer.from(buildActionCursorArmScript(kind), 'utf8').toString('base64')
}
