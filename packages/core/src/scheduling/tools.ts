...
export type SchedulingToolDeps = {
   ...
   * messaging channel (telegram/slack/whatsapp/feishu) posts a one-line confirmation
...
export function createSchedulingTools(deps: SchedulingToolDeps): {
  ...
  const createScheduledJob = buildTool({
    ...
    inputSchema: z.object({
      ...
      deliveryChannel: z.enum(['telegram', 'slack', 'whatsapp', 'custom', 'feishu']).optional().describe('Channel TYPE to deliver the result to (custom = a bridge-driven custom channel; feishu covers both Feishu China and international Lark). Omit to use the preferred messaging channel (auto-detected). When set to a messaging channel while the user is chatting on that channel, delivery is pinned to the EXACT current chat — including the Telegram forum topic the user is in — captured automatically from the session. You do not (and cannot) pass a chat id or topic id; this enum is type-only by design. Setting a messaging channel also posts a confirmation ping into that chat/topic so the user can see where the update will land.\n\nResults are NEVER delivered to the web chat (the web UI is a pull surface, not a notification channel). If the user has no messaging channel connected and is not scheduling a doc-page update, the job cannot be created — tell them to connect Telegram, Slack, WhatsApp, or Feishu/Lark first. When the user schedules from a doc page, the job updates that page in place and delivers nothing to a channel.'),
