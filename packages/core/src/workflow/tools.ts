...
export type WorkflowToolDeps = {
  ...
  /** Enumerates the only stable keys accepted by CRM event `match.tags`. */
...
export const TRIGGER_INPUT_DESCRIPTION =
  ...
  `Shapes: \`{ type: "connector", connectorInstanceId, provider }\` (a CONNECTED connector instance), \`{ type: "channel", channelIntegrationId, channel }\` (a connected chat integration; get channelIntegrationId from listChannels.integrationId, never listChannels.id), \`{ type: "page", pageId }\` (a doc page + its direct children), \`{ type: "task" }\` (the workspace's tasks — id-less), \`{ type: "knowledge" }\` (the workspace's knowledge base — id-less), \`{ type: "brand" }\` (workspace brand — id-less), \`{ type: "crm" }\` (committed CRM operations — id-less). ` +
  `\`match\` narrows firing: keywords / fromActors / inChannels / mentions / tags (task, knowledge, brand, and CRM events) / currentTags (task only, checks the task's full live tag set) / fromBots (default false — bot- or assistant-authored events only fire with \`fromBots: true\`). ` +
...
export function createWorkflowTools(deps: WorkflowToolDeps): {
  ...
  const proposeWorkflow = buildTool({
    ...
    description:
      ...
      `\n\nExternal-client email replies: when the user says to inherit a Studio assistant's public API-key client boundary, pass \`clientBoundary\` with that concrete assistant UUID and a static resolver, an administrator-authored sender-to-externalUserId map, or \`{ kind: "verified_email_pairing" }\`, and OMIT \`definition.principal\`; the server resolves only safe key metadata and never exposes the secret. Verified email pairing fails unless the inbound sender has exactly one pairing written by an identified turn on that exact key and assistant; it never searches workspace CRM. The model step stays read-only and draft-only. To require human review before replying, store that draft and add exactly one \`imapSendMessage\` tool_call with \`to: ["{{input.event.sender}}"]\`, a subject containing \`{{input.event.subject}}\`, \`body: "{{vars.<draft>}}"\`, \`inReplyTo: "{{input.event.message_id}}"\`, no Cc/Bcc/attachments/explicit From, and \`approval: { required: true }\`. This always pauses in Approvals even if connector policy is Allow; never promise automatic sending. ` +
    ...
    async execute(input, context) {
      ...
      if (trigger?.kind === 'event' && deps.listCrmWorkflowEventFilterKeys) {
        ...
        if (requested.length > 0) {
          let valid: string[]
          try {
            valid = await deps.listCrmWorkflowEventFilterKeys(context.workspaceId!)
          } catch {
            return {
              data: { ok: false, errors: ['The CRM workflow event catalog could not be read. Retry after CRM is available.'] },
              isError: true,
            }
          }
          ...
          if (unknown.length > 0) {
            return {
              data: {
                ok: false,
                errors: [`Unknown CRM workflow filter key(s): ${unknown.join(', ')}. Valid values: ${valid.slice(0, 500).join(', ')}.`],
              },
              isError: true,
            }
          }
