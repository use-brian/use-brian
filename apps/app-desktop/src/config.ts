...
// The authenticated product is served at `app.sidan.ai`. `deriveApiUrl`
// (target-store.ts) maps the `app.` host to the sibling `api.` backend (the
// legacy `canvas.` prefix is kept as a tolerant fallback). `SIDANCLAW_APP_URL`
...
export function resolveConfig(
  ...
  const envAppUrl = env.SIDANCLAW_APP_URL?.trim() || "";
  ...
  const apiUrl = (env.SIDANCLAW_API_URL?.trim() || target?.apiUrl || deriveApiUrl(appUrl)).replace(
    /\/+$/,
    "",
  );
