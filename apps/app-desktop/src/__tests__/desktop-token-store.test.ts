...
describe("[COMP:app-desktop/token-store] persistTokens", () => {
  it("writes and verifies an encrypted session", () => {
  ...
  it("refuses to persist without OS encryption", () => {
    let wrote = false;
    expect(
      persistTokens(
        fakeCipher(false),
  ...
  it("fails when the written session cannot be read back", () => {
    expect(
