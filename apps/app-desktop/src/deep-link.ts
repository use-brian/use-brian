...
const MAX_SIRI_PROMPT_LENGTH = 8_000;

/** Return a bounded Siri prompt only from the dedicated `ask` deep link. */
export function parseAskBrianDeepLink(rawUrl: string, protocolScheme: string): string | null {
  let url: URL;
  try {
    url = new URL(rawUrl);
  } catch {
  ...
  if (url.protocol !== `${protocolScheme}:` || url.hostname !== "ask") return null;
