...
describe("[COMP:app-web/plan-gate] Plan gate decision", () => {
  const hosted = deploymentCapabilitiesFor("hosted");
  const oss = deploymentCapabilitiesFor("oss");
  const outpost = deploymentCapabilitiesFor("outpost");
  ...
  it("never gates Outpost despite persisted hosted plan state", () => {
    expect(planGateApplies(outpost, "free")).toBe(false);
    expect(modelTierPlanGateApplies(outpost, "free", "pro")).toBe(false);
    expect(modelTierPlanGateApplies(outpost, "free", "max")).toBe(false);
  });
