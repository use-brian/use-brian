...
export function OfficeReview({ artifact, artifactId, workspaceId, selectedObjectIds, onLifecycle, offlineCopy = false }: { artifact: OfficeArtifact; artifactId: string; workspaceId: string; selectedObjectIds: string[]; onLifecycle(artifact: OfficeArtifact): void; offlineCopy?: boolean }) {
  ...
  return <div className="space-y-5 p-4 text-sm">
    ...
    <section className="space-y-3">
      ...
      <select aria-label={t.releaseAction} value={action} onChange={(event) => setAction(event.target.value as OfficeReleaseInput["action"])} className="h-9 w-full rounded border bg-background px-2"><option value="export">{t.export}</option><option value="share">{t.share}</option><option value="present">{t.present}</option><option value="send">{t.send}</option><option value="publish">{t.publish}</option></select>
      <select aria-label={t.destinationSensitivity} value={sensitivity} onChange={(event) => setSensitivity(event.target.value as typeof sensitivity)} className="h-9 w-full rounded border bg-background px-2"><option value="public">{t.public}</option><option value="internal">{t.internal}</option><option value="confidential">{t.confidential}</option></select>
