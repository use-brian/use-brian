...
export function OfficeReview({ artifact, artifactId, workspaceId, selectedObjectIds, onLifecycle, offlineCopy = false }: { artifact: OfficeArtifact; artifactId: string; workspaceId: string; selectedObjectIds: string[]; onLifecycle(artifact: OfficeArtifact): void; offlineCopy?: boolean }) {
  ...
  return <div className="space-y-5 p-4 text-sm">
    ...
    <section className="space-y-3">
      ...
      <div className="flex gap-2"><button type="button" disabled={busy || offlineCopy} onClick={() => void runReview()} className="rounded border px-3 py-2 disabled:opacity-50">{t.review}</button>{receipt && receipt.status !== "blocked" ? <button type="button" disabled={busy || offlineCopy} onClick={() => void runRelease()} className="rounded bg-action px-3 py-2 text-action-foreground disabled:opacity-50">{receipt.status === "needs_ack" ? t.acknowledgeRelease : t.release}</button> : null}</div>
